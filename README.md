...
### 2025-04-07 - v0.1.4
**fix: 修复 type.ts 依赖问题，完善 builtin.ts 工具实现**
- 修复 `tools/type.ts` 中的类型依赖问题
- 完善 `tools/builtin.ts` 中的内置工具定义
- 添加更多工具函数的实现逻辑
- 优化类型导出和导入

### 2025-04-07 - v0.1.3
**fix: 修复并更新沙箱路径和工具相关代码**
- 完善 `sandbox-paths.ts` 中的路径安全检查功能
- 实现 `tools/builtin.ts` 中的内置工具定义
- 完善 `tools/type.ts` 中的工具类型定义
- 添加 305 行代码，增强工具系统功能

### 2025-04-06 - v0.1.2
**fix: 修复 assertSandboxPath 函数中的变量名和类型定义错误**
- 修正返回类型定义（`resolve` → `resolved`）
- 统一变量命名（`resolved_and_relative`）
- 修复变量引用错误
...
### 3. 工具系统
- [x] 工具类型定义（`Tool`, `ToolContext`, `ToolResult`）
